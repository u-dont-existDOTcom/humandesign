"""Adaptive chart-blind relationship capture with LLM answer auditing.

The LLM layer receives questionnaire semantics and participant responses only. It
never receives birth data, charts, candidate identities, AstroRRF predictions, or
model fit. Participant responses are sent to the configured provider only after
explicit LLM-processing consent.
"""
...
def _next_audit_item(audit: dict[str, Any]) -> LLMClarificationItem | None:
    answered = {str(row["clarification_id"]) for row in _audit_answers(audit)}
...
def _prior_clarifications(payload: dict[str, Any]) -> tuple[dict[str, Any], ...]:
    ...
    for answer in cast(list[dict[str, Any]], old.get("answers", [])):
        source = queue_lookup.get(str(answer.get("clarification_id", "")), {})
        rows.append(
            {
...
def _build_llm_audit(
    ...
    *,
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail="The LLM auditor failed. Saved answers were not changed; try again.",
        ) from exc
...
def create_relationship_adaptive_app_from_env() -> FastAPI:
    ...
    async def adaptive_integrity_guard(
        ...
        if request.method == "POST" and request.url.path == "/api/sessions":
            return JSONResponse(
                status_code=410,
                content={"detail": "use the LLM-consented adaptive session endpoint"},
            )
