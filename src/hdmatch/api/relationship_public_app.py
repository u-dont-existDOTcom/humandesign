...
<script>
let sessionId=localStorage.getItem('rr_session');let token=localStorage.getItem('rr_token');let current=null;let mode='main';let editing=false;let state=null;
...
async function editDomain(qid){const d=await loadState();const list=mode==='clarification'?d.clarification_addendum.answers:d.answers;const existing=list.find(a=>a.question_id===qid);const q=(await questionFor(qid,d));editing=true;showQuestion(q,existing)}
async function questionFor(qid,d){let q=d.next_question;if(q&&q.id===qid)return q;if(d.clarification_next_question&&d.clarification_next_question.id===qid)return d.clarification_next_question;const endpoint='/api/sessions/'+sessionId+'?token='+encodeURIComponent(token);const r=await fetch(endpoint);const fresh=await r.json();const candidates=[fresh.next_question,fresh.clarification_next_question].filter(Boolean);const found=candidates.find(x=>x.id===qid);if(found)return found;const source=mode==='clarification'?fresh.clarification_addendum.answers:fresh.answers;const answer=source.find(x=>x.question_id===qid);if(!answer)throw new Error('Question not found');return questionDefinitions[qid]||await fetchQuestionDefinition(qid)}
let questionDefinitions={};async function fetchQuestionDefinition(qid){const r=await fetch('/api/sessions/'+sessionId+'?token='+encodeURIComponent(token));const d=await r.json();if(d.next_question)questionDefinitions[d.next_question.id]=d.next_question;if(d.clarification_next_question)questionDefinitions[d.clarification_next_question.id]=d.clarification_next_question;if(questionDefinitions[qid])return questionDefinitions[qid];throw new Error('Reload the page and try editing again.')}
