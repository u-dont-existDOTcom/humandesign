...
def load_model() -> tuple[dict[str, Any], tuple[dict[str, Any], ...], dict[str, Any]]:
    ...
    lookup = {
        row["rule_id"]: row for row in registry(source, {d["domain_id"] for d in source["domains"]})
    }
...
def answer_signs(profile: Mapping[str, str]) -> tuple[int, ...]:
    ...
    for rule in load_model()[2]["rules"]:
        ...
        signs.append(
            1 if "supported" in states else -1 if all(s == "contradicted" for s in states) else 0
        )
    return tuple(signs)
...
def candidate_minutes(count: int) -> tuple[int, ...]:
    ...
    reservoir = random.Random(150925).sample(
        range(int((END - START).total_seconds() // 60) + 1), MAX_DECOYS
    )
...
def validate_location(latitude: float, longitude: float) -> None:
    ...
    if not -66 <= latitude <= 66 or not -180 <= longitude <= 180:
        raise ValueError(
            "This prototype supports birthplaces between 66 degrees south and north; polar-house behavior is unresolved"  # noqa: E501 - preserve human-facing wording
        )


def _mask_unlocked(when: datetime, latitude: float, longitude: float, ephemeris_root: Path) -> int:
    snapshot = build_snapshot(
        when, latitude=latitude, longitude=longitude, ephemeris_root=ephemeris_root
    )
...
def panel_masks(
    count: int, latitude: float, longitude: float, ephemeris_root: str
) -> tuple[int, ...]:
    validate_location(latitude, longitude)
    with _ASTRO_LOCK:
        return tuple(
            _mask_unlocked(t, latitude, longitude, Path(ephemeris_root))
            for t in candidate_times(count)
        )
...
def rank_against(scores: Sequence[int], score: int) -> dict[str, Any]:
    ...
    return {
        ...
        "rank_mid": higher + ties / 2 + 1,
        "higher_candidates": higher,
        "tied_other_candidates": ties,
        "comparison_count_including_checked_instant": len(scores) + 1,
        "unique_first": higher == 0 and ties == 0,
        "joint_first": higher == 0,
    }
...
def run_panel(
    ...
    *,
    ...
    if not any(signs):
        raise ValueError(
            "No scored evidence: all active interpretations are mixed, unknown, or not applicable"
        )
    ...
    model_id = content_hash(
        {
            ...
            "bridge": file_hash(BRIDGE_PATH),
            "scorer": file_hash(Path(__file__)),
        }
    )
    return {
        "model_id": model_id,
        "model_version": "1.5.0-experimental-answer-bridge",
        "profile_sha256": content_hash(profile),
        "answer_signs": list(signs),
        "panel_sha256": content_hash([t.isoformat() for t in times]),
        "decoy_count": count,
        "panel_seed": 150925,
        "python_version": platform.python_version(),
        "ephemeris_engine_version": swe.version,
        "ephemeris_sha256": {
            name: file_hash(ephemeris_root / name) for name in ("sepl_18.se1", "semo_18.se1")
        },
        "universe_start": START.isoformat(),
        "universe_end": END.isoformat(),
        "birthplace": {"latitude": latitude, "longitude": longitude},
        "score_histogram": dict(sorted(Counter(scores).items())),
        "maximum_observed_score": best,
        "maximum_observed_count": scores.count(best),
        "top_candidates": [{"utc": times[i].isoformat(), "score": scores[i]} for i in order[:20]],
        "claim": "Frozen-model candidate-panel test; not an exhaustive century search, probability, or validation result",  # noqa: E501 - preserve human-facing wording
        "reference_fit": "The six astronomical conditions were selected on one known development person",  # noqa: E501 - preserve human-facing wording
...
def check_instant(
    ...
    *,
    ...
    other_scores = [score_mask(m, signs) for t, m in zip(times, masks, strict=True) if t != when]
    mask = mask_at(when, latitude, longitude, ephemeris_root)
    score = score_mask(mask, signs)
    return {
        "checked_utc": when.astimezone(UTC).isoformat(),
        "score": score,
        "rule_bits": [(mask >> i) & 1 for i in range(6)],
        **rank_against(other_scores, score),
