...
This module never sees birth data, charts, candidate identities, or model fit. It
only judges whether a response is usable for the frozen relationship phenotype
vocabulary and generates bounded clarification prompts for known ambiguities.
"""
...
_DIRECT_SCALAR_FIELDS = {
    ...
    "b_commitment_intent",
    "a_baseline_libido",
    "a_partner_specific_desire",
    "b_baseline_libido",
    "b_partner_specific_desire",
    "a_sexual_satisfaction",
    "b_sexual_satisfaction",
    "dyadic_sexual_chemistry",
...
def assess_field_answer(
    ...
    """Score answer usability without seeing chart/model information."""
    ...
    if status in {"unknown", "not_applicable"}:
        return AnswerQuality(
            ...
            hints=("Explicit unknown/not-applicable is complete when it is accurate.",),
    ...
    codes: list[str] = []
    hints: list[str] = []
    concise_scalar = field_id in _DIRECT_SCALAR_FIELDS and _contains_any(lowered, _DIRECT_LEVEL)
    ...
    if field_id in _PARTNER_INFERENCE_FIELDS and _contains_any(lowered, _PARTNER_EVIDENCE):
        score += 8
    elif field_id in _PARTNER_INFERENCE_FIELDS and (
        _contains_any(lowered, _HEDGES) or word_count < 12
    ):
    ...
    if word_count < 8 and not codes and not concise_scalar:
        codes.append("low_detail")
        score -= 10

    if status in {"mixed", "context_dependent"} and clarification_clean:
        score += min(len(_TOKEN_RE.findall(clarification_clean)), 10)
    elif status in {"mixed", "context_dependent"}:
        score -= 20
...
def _apply_field_specific_rules(
    ...
    if (
        field_id in {"a_eros_in_love", "b_eros_in_love"}
        and _contains_any(lowered, ("love", "loved", "care", "attached"))
        and not _contains_any(lowered, ("in love", "romantic", "longing", "romantically"))
    ):
        codes.append("love_eros_conflation")
    if (
        field_id in {"a_physical_attraction", "b_physical_attraction"}
        and _contains_any(lowered, ("sex", "chemistry", "sexual"))
        and not _contains_any(
            lowered,
            ("attract", "physical", "appearance", "looks", "hot", "beautiful", "handsome"),
        )
    ):
        codes.append("attraction_chemistry_conflation")
    if field_id in {"a_baseline_libido", "b_baseline_libido"} and not _contains_any(
        lowered,
        ("general", "overall", "libido", "sexual appetite", "other partner", "normally"),
    ):
        codes.append("libido_partner_desire_conflation")
    if (
        field_id in {"intellectual_compatibility", "conceptual_comprehension"}
        and _contains_any(lowered, ("smart", "intelligent", "educated", "iq"))
        and not _contains_any(
            lowered,
            ("reason", "understand", "disagree", "idea", "concept", "explain", "apply"),
        )
    ):
        codes.append("intellect_interaction_conflation")
    if (
        field_id == "intellectual_stimulation"
        and _contains_any(lowered, ("good conversation", "talked", "smart", "compatible"))
        and not _contains_any(
            lowered,
            ("new", "surprise", "expand", "changed my", "learned", "challenged", "new way"),
        )
    ):
        codes.append("stimulation_compatibility_conflation")
    if (
        field_id == "communication_quality"
        and _contains_any(lowered, ("a lot", "often", "hours", "every day", "constantly"))
        and not _contains_any(
            lowered,
            ("clear", "honest", "understood", "easy", "productive", "misunderstand"),
        )
    ):
        codes.append("communication_quality_amount_conflation")
    if (
        field_id in {"visible_drama", "serious_conflict"}
        and _contains_any(lowered, ("drama", "dramatic", "fight", "fought", "conflict"))
        and not _contains_any(
            lowered,
            ("playful", "theatrical", "hostile", "harmful", "aggression", "contempt", "coerc"),
        )
    ):
        codes.append("drama_conflict_conflation")
    if (
        field_id in {"sexual_jealousy", "romantic_priority_jealousy"}
        and "jealous" in lowered
        and not _contains_any(
            lowered,
            ("sex", "sexual", "sleep with", "in love", "romantic", "priority", "attention"),
        )
    ):
        codes.append("jealousy_type_conflation")
    if (
        field_id in {"practical_life_fit", "continuation_breakup_factors"}
        and word_count < 14
        and not _contains_any(
            lowered,
            (
