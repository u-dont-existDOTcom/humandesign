...
| Free text | Narratives contain names, places, dates, relationship facts, or distinctive phrases. | No public free text, excerpts, generated summaries of participant text, or row-level error notes. | Structural prohibition present. |
...
The synthetic schema permits only overall eligible/included counts; aggregate intersection and abstention counts; aggregate date-coverage counts; separate cohort summaries of temporal-width and full-state-count retained ratios; nonpersonal protocol/method/software provenance; and an explicit disclosure-review block. It does not permit arbitrary properties.
