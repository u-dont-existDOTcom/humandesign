...
def main():
    ...
    with z.open(csv_names[0]) as fb:
        ...
        mother_years = Counter()
        father_years = Counter()
        wedding_years = Counter()
        id_nonempty = 0
        id2_nonempty = 0
        ids = set()
        id2s = set()
        ...
        marriage_age_m_bins = Counter()
        marriage_age_f_bins = Counter()
        for row in reader:
            row_count += 1
            md = dob_tuple(row, "m")
            fd = dob_tuple(row, "f")
            wd = wedding_tuple(row)
            if md and fd:
                both_parent_dob += 1
                mother_years[md[0]] += 1
                father_years[fd[0]] += 1
            ...
            if wd:
                with_wedding += 1
                wedding_years[wd[0]] += 1
            if md and fd and wd:
                all_three += 1
                pairwedd = (md, fd, wd)
                pairweddkeys.add(pairwedd)
                if pairwedd in seen_pairwedd:
                    duplicate_pairwedd_rows += 1
                seen_pairwedd.add(pairwedd)
                mdate = date(*md)
                fdate = date(*fd)
                wdate = date(*wd)
                mage = (wdate - mdate).days / 365.2425
                fage = (wdate - fdate).days / 365.2425
                if mage < 0 or fage < 0:
                    wedding_before_birth += 1
                if 14 <= mage <= 85 and 14 <= fage <= 85:
                    plausible_all_three += 1
                    parent_age_diff_bins[
                        str(int(abs((mdate - fdate).days) / 365.2425) // 5 * 5)
                    ] += 1
                    marriage_age_m_bins[str(int(mage) // 5 * 5)] += 1
                    marriage_age_f_bins[str(int(fage) // 5 * 5)] += 1
            idv = (row.get("id") or "").strip()
            id2v = (row.get("id2") or "").strip()
            if idv:
                id_nonempty += 1
                ids.add(idv)
            if id2v:
    ...
    summary = {
        ...
        "coarse_distributions": {
            "absolute_parent_age_difference_5y_bins": dict(
                sorted(parent_age_diff_bins.items(), key=lambda kv: int(kv[0]))
            ),
            "mother_age_at_wedding_5y_bins": dict(
                sorted(marriage_age_m_bins.items(), key=lambda kv: int(kv[0]))
            ),
            "father_age_at_wedding_5y_bins": dict(
                sorted(marriage_age_f_bins.items(), key=lambda kv: int(kv[0]))
            ),
        },
        "source_notice": [
            "Data are Didier Castille adaptations of INSEE files, not official INSEE files.",
            (
                "Source README states the files are neither official nor "
                "scientifically valid and depend on Castille's good faith"
                "."
            ),
            "Birth and wedding data are untimed.",
        ],
        "next_decision": (
            "Use audit to freeze deduplication and independent untime"
            "d partner-selection / wedding-timing tests; do not inspe"
            "ct astrology before freeze."
        ),
    }
