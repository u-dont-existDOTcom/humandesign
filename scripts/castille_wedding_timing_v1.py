...
ASPECTS = np.asarray([0.0, 60.0, 90.0, 120.0, 180.0], dtype=np.float64)
SIGMA = 3.0
    ...
    for dy in SHIFTS:
        ...
        try:
            ...
            if m == 2 and d == 29:
                ...
                continue
        cand = (y, m, d)
        ma = static.age_years(r.mother, cand)
        fa = static.age_years(r.father, cand)
        if 14 <= ma <= 85 and 14 <= fa <= 85:
            vals.append((dy, cand, adjusted))
    return vals
...
def calc(jd: float, body: int) -> float:
    ...
    if used != swe.FLG_SWIEPH:
        raise RuntimeError(f"EPHEMERIS_FALLBACK jd={jd} body={body} used={used} ret={ret}")
...
def natal_positions(
    ...
    for i, d in enumerate(sorted(dates), 1):
        ...
        if i % 5000 == 0:
            print(f"natal cache {i}/{len(dates)} hour={hour}", flush=True)
    return out
...
def transit_positions(dates: set[tuple[int, int, int]]) -> dict[tuple[int, int, int], np.ndarray]:
    ...
    for i, d in enumerate(sorted(dates), 1):
        ...
        if i % 5000 == 0:
            print(f"transit cache {i}/{len(dates)}", flush=True)
    return out


def progressed_positions(
    birth: tuple[int, int, int], cand: tuple[int, int, int], birth_hour: float
) -> np.ndarray:
    bj = jd_date(birth, birth_hour)
    cj = jd_date(cand, 12.0)
    age = (cj - bj) / TROPICAL_YEAR
    pj = bj + age
...
def aspects(movers: np.ndarray, targets: np.ndarray) -> np.ndarray:
    diff = (
        movers[:, None].astype(np.float64) - targets[None, :].astype(np.float64) + 180.0
    ) % 360.0 - 180.0
...
def base_features(r: TRecord, cand: tuple[int, int, int]) -> np.ndarray:
    ...
    return np.asarray(
        [ma, fa, signed, abs(signed), (cand[0] - 1985.0) / 20.0, math.sin(phase) + math.cos(phase)],
        dtype=np.float32,
    )
...
def row_features(
    ...
    dynamic_shift_years: int = 0,
) -> np.ndarray:
    f = np.empty(P3, dtype=np.float32)
    f[:P0] = base_features(r, cand)
    off = P0
    # Astrology can be deliberately time-shifted for the frozen falsification while baseline
    # remains at cand.
    dyn = cand
    ...
    nb = natal[r.father]
    tr = transits[dyn]
    pa = progressed_positions(r.mother, dyn, birth_hour)
    pb = progressed_positions(r.father, dyn, birth_hour)
    ...
    f[off : off + len(x)] = x
    off += len(x)
    ncomp = midpoint(na, nb)
    x = aspects(tr, ncomp)
    f[off : off + len(x)] = x
    off += len(x)
    pcomp = midpoint(pa, pb)
    x = aspects(tr, pcomp)
    f[off : off + len(x)] = x
    off += len(x)
    if off != P3:
        raise RuntimeError(f"feature size mismatch {off} vs {P3}")
    return f
...
def collect_dates(cases, risk_rows=None, shift3=False):
    ...
    events = set()
    for r, c in cases:
        births.update((r.mother, r.father))
        events.update((r.wedding, c))
    if risk_rows:
        for r in risk_rows:
            births.update((r.mother, r.father))
            events.add(r.wedding)
...
def build_binary_matrix(cases, natal, transits, birth_hour=12.0, dynamic_shift_years=0):
    X = np.empty((len(cases) * 2, P3), dtype=np.float32)
    y = np.empty(len(cases) * 2, dtype=np.int8)
    for i, (r, c) in enumerate(cases):
        X[2 * i] = row_features(r, r.wedding, natal, transits, birth_hour, dynamic_shift_years)
        y[2 * i] = 1
        X[2 * i + 1] = row_features(r, c, natal, transits, birth_hour, dynamic_shift_years)
        y[2 * i + 1] = 0
        if (i + 1) % 5000 == 0:
            print(f"timing binary features {i + 1}/{len(cases)}", flush=True)
    return X, y


def scaler(B):
    mean = B[:, :P0].mean(axis=0, dtype=np.float64).astype(np.float32)
    sd = B[:, :P0].std(axis=0, dtype=np.float64).astype(np.float32)
    sd[sd < 1e-8] = 1
    return mean, sd


def scale_inplace(X, mean, sd):
    X[:, :P0] = (X[:, :P0] - mean) / sd
...
def metrics(y, dec):
    ...
    return {
        "roc_auc": float(roc_auc_score(y, dec)),
...
def select_alpha(Xd, yd, Xv, yv, dim):
    trials = []
    for a in ALPHAS:
        clf = new_model(a).fit(Xd[:, :dim], yd)
        dec = clf.decision_function(Xv[:, :dim])
        met = metrics(yv, dec)
    ...
    best = max(t["roc_auc"] for t in trials)
    elig = [t for t in trials if best - t["roc_auc"] <= 1e-4]
    sel = max(elig, key=lambda t: t["alpha"])
    return {"trials": trials, "selected_alpha": sel["alpha"], "selected_validation": sel}


def fit_final(Xtrain, ytrain, Xfinal, yfinal, dim, alpha):
    clf = new_model(alpha).fit(Xtrain[:, :dim], ytrain)
    dec = clf.decision_function(Xfinal[:, :dim])
    met = metrics(yfinal, dec)
    cal = LogisticRegression(penalty=None, solver="lbfgs", max_iter=2000).fit(
        dec.reshape(-1, 1), yfinal
    )
    met.update(
        {
            "calibration_intercept": float(cal.intercept_[0]),
            "calibration_slope": float(cal.coef_[0][0]),
            "nonzero": int(np.sum(np.abs(clf.coef_[0]) > 1e-10)),
        }
    )
    return met, clf, dec
...
def risk_metrics(
    rows, natal, transits, clf, mean, sd, dim, birth_hour=12.0, dynamic_shift_years=0, batch=250
):
    ranks = []
    pcts = []
    used = 0
    ...
    for start in range(0, len(rows), batch):
        ...
        for r in chunk:
            ...
            arr = np.vstack(
                [
                    row_features(r, c, natal, transits, birth_hour, dynamic_shift_years)
                    for c in cands
                ]
            )
            arr[:, :P0] = (arr[:, :P0] - mean) / sd
            feat.append(arr)
            sizes.append(len(cands))
        if not feat:
            continue
        big = np.vstack(feat)
        scores = clf.decision_function(big[:, :dim])
        pos = 0
        for n in sizes:
            arr = scores[pos : pos + n]
            pos += n
            true = float(arr[0])
            ctl = arr[1:]
            higher = int(np.sum(ctl > true + 1e-12))
            tied = int(np.sum(np.abs(ctl - true) <= 1e-12))
            rank = 1 + higher + 0.5 * tied
            pct = 100 * (len(ctl) - higher - 0.5 * tied) / len(ctl)
            ranks.append(rank)
            pcts.append(pct)
            used += 1
    ...
    return {
        ...
        "top3_rate": float(np.mean([r <= 3 + 1e-12 for r in ranks])),
        "mean_reciprocal_rank": float(np.mean([1 / r for r in ranks])),
    }


def permutation(dec, n_pairs=20_000, n=200):
    k = min(n_pairs, len(dec) // 2)
    scores = dec[: 2 * k].reshape(k, 2)
    obs = float(roc_auc_score(np.tile([1, 0], k), scores.reshape(-1)))
    rng = random.Random(SEED)
    vals = []
    for _ in range(n):
        lab = np.empty((k, 2), dtype=np.int8)
        for i in range(k):
            lab[i] = [1, 0] if rng.random() < 0.5 else [0, 1]
...
def main():
    ...
    records, raw_bytes, audit = timing_records()
    splits = cap_splits(records)
    cases = {}
    drops = {}
    for s, rs in splits.items():
        cases[s], drops[s] = make_binary_cases(rs)
        print(s, len(rs), len(cases[s]), flush=True)
    rr = risk_rows(records)
    births, events = collect_dates(
        cases["discovery"] + cases["validation"] + cases["final"], rr, True
    )
    natal = natal_positions(births, 12.0)
    transits = transit_positions(events)
    Xd, yd = build_binary_matrix(cases["discovery"], natal, transits)
    Xv, yv = build_binary_matrix(cases["validation"], natal, transits)
    Xf, yf = build_binary_matrix(cases["final"], natal, transits)
    Bd = Xd[:, :P0].copy()
    Bv = Xv[:, :P0].copy()
    Bf = Xf[:, :P0].copy()
    md, sd = scaler(Xd)
    scale_inplace(Xd, md, sd)
    scale_inplace(Xv, md, sd)
    selections = {m: select_alpha(Xd, yd, Xv, yv, dim) for m, dim in MODEL_DIMS.items()}
    Btrain = np.vstack([Bd, Bv])
    mr = Btrain.mean(axis=0, dtype=np.float64).astype(np.float32)
    sr = Btrain.std(axis=0, dtype=np.float64).astype(np.float32)
    sr[sr < 1e-8] = 1
    Xd[:, :P0] = (Bd - mr) / sr
    Xv[:, :P0] = (Bv - mr) / sr
    Xf[:, :P0] = (Bf - mr) / sr
    Xtrain = np.vstack([Xd, Xv])
    ytrain = np.concatenate([yd, yv])
    del Xd, Xv, Bd, Bv, Btrain
    final = {}
    models = {}
    decisions = {}
    for m, dim in MODEL_DIMS.items():
        met, clf, dec = fit_final(Xtrain, ytrain, Xf, yf, dim, selections[m]["selected_alpha"])
        final[m] = met
        models[m] = clf
        decisions[m] = dec
    risk = {
        m: risk_metrics(rr, natal, transits, models[m], mr, sr, dim)
        for m, dim in MODEL_DIMS.items()
    }
    d1 = final["M1T"]["roc_auc"] - final["M0T"]["roc_auc"]
    l1 = final["M1T"]["log_loss"] - final["M0T"]["log_loss"]
    r1 = risk["M1T"]["mean_true_date_percentile"]
    d3 = final["M3T"]["roc_auc"] - final["M1T"]["roc_auc"]
    l3 = final["M3T"]["log_loss"] - final["M1T"]["log_loss"]
    r3 = risk["M3T"]["mean_true_date_percentile"] - risk["M1T"]["mean_true_date_percentile"]
    promising1 = final["M1T"]["roc_auc"] >= 0.52 and d1 >= 0.01 and l1 <= -0.002 and r1 >= 55
    promising3 = (
        final["M3T"]["roc_auc"] >= 0.52
        and d3 >= 0.01
        and l3 <= -0.002
        and risk["M3T"]["mean_true_date_percentile"] >= 55
        and r3 >= 5
    )
    strong3 = d3 >= 0.03 and r3 >= 10
    selected = "M3T" if d3 >= d1 else "M1T"
    perm = permutation(decisions[selected])
    # +3y falsification, same primary noon natal cache and frozen models; baseline remains
    # candidate date.
    Xshift, yshift = build_binary_matrix(cases["final"], natal, transits, 12.0, 3)
    Xshift[:, :P0] = (Bf - mr) / sr
    shift = {
        m: metrics(yshift, models[m].decision_function(Xshift[:, :dim]))
        for m, dim in MODEL_DIMS.items()
        if m != "M0T"
    }
    # Birth-time sensitivity uses same trained models, recomputed natal/progressions at alternate
    # birth-date hours.
    sensitivity = {}
    for label, hour in (("00UTC", 0.0), ("2359UTC", 23 + 59 / 60)):
        nalt = natal_positions(births, hour)
        Xalt, yalt = build_binary_matrix(cases["final"], nalt, transits, hour)
        Xalt[:, :P0] = (Bf - mr) / sr
        sensitivity[label] = {
            m: metrics(yalt, models[m].decision_function(Xalt[:, :dim]))
            for m, dim in MODEL_DIMS.items()
            if m != "M0T"
        }
    out = {
        "status": "independent_large_n_wedding_timing_final_complete",
        "freeze_spec": str(FREEZE.relative_to(REPO)),
        "freeze_sha256": static.sha256_file(FREEZE),
        "source": static.URL,
        "source_raw_bytes": raw_bytes,
        "data_audit": audit,
        "split_counts": {
            s: {"capped_real": len(splits[s]), "binary_cases": len(cases[s]), "drop": drops[s]}
            for s in splits
        },
        "feature_dimensions": MODEL_DIMS,
        "selections": selections,
        "final_binary": final,
        "final_full_timing_risk_set": risk,
        "incremental": {
            "M1T_delta_auc_vs_M0T": d1,
            "M1T_delta_logloss_vs_M0T": l1,
            "M1T_promising": promising1,
            "M3T_delta_auc_vs_M1T": d3,
            "M3T_delta_logloss_vs_M1T": l3,
            "M3T_delta_risk_percentile_vs_M1T": r3,
            "M3T_promising": promising3,
            "M3T_strong": strong3,
        },
        "selected_for_permutation": selected,
        "permutation_diagnostic": perm,
        "time_shift_plus3y_falsification": shift,
        "birth_time_sensitivity": sensitivity,
        "ephemeris": {
            "requested": "SWIEPH",
            "returned": "SWIEPH or abort",
            "sepl_18_sha256": static.sha256_file(EPHE / "sepl_18.se1"),
            "semo_18_sha256": static.sha256_file(EPHE / "semo_18.se1"),
        },
        "interpretation_rule": (
            "Promote only families meeting frozen promising threshold"
            "s; otherwise preserve negative/small effect without coef"
            "ficient mining."
        ),
    }
    ...
    print("wrote", OUT, "sha256", static.sha256_file(OUT), flush=True)
