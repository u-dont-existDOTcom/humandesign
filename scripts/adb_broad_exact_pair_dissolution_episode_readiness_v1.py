...
def h3_current_marker(entry: str, is_template: bool, other_wiki: str | None, other_name: str):
    ...
    if is_template:
        ...
        if any(
            wp.REASON_ONLY_RE.fullmatch(wp.plain(v))
        ...
        return {
            "source_family": "H3",
            "source": "H3_wikipedia_current_open",
    ...
    return {
        "source_family": "H3",
        "source": "H3_wikipedia_current_open_plain",
        "start": {k: start[k] for k in ("precision", "interval_start", "interval_end", "text")},
        "raw": entry,
    }
...
def fetch_h3_current_markers(universe: dict, h3: dict):
    ...
    for i in range(0, len(titles), batch_size):
        ...
        if (i // batch_size + 1) % 5 == 0:
            print(
                (
                    f"episode H3 current-marker batches "
                    f"{i // batch_size + 1}/{(len(titles) + batch_size - 1) // batch_size}"
                ),
                flush=True,
            )
        time.sleep(0.35)
    ...
    for up in universe["pairs"]:
        pk = up["pair_key"]
        hp = h3_by[pk]
        a = int(up["person_a"]["adb_id"])
        b = int(up["person_b"]["adb_id"])
        ia = hp.get("wikipedia_identity_a") or {}
        ib = hp.get("wikipedia_identity_b") or {}
        entries = []
        for src, other, sident, oident, direction in (
            ...
            if not canonical or not wt:
                ...
                continue
            ibox = wp.first_infobox(wt)
            fields = wp.top_fields(ibox) if ibox else {}
                        ...
                        marker.update(
                            {
                                ...
                                "source_wikipedia_title": canonical,
                            }
                        )
...
def h4_all_statements(p4: dict):
    ...
    for x in rows:
        k = (
            x.get("source_qid"),
            x.get("other_qid"),
            x.get("relationship_property"),
            x.get("statement_index"),
            x.get("direction"),
        )
        if k not in seen:
            seen.add(k)
...
def collect_entries(p12: dict, p4: dict):
            ...
            out.append(
                {
                    ...
                    "statement": {
                        "source_qid": st.get("source_qid"),
                        "other_qid": st.get("other_qid"),
                        "direction": st.get("direction"),
                        "property": st.get("relationship_property"),
                        "statement_index": st.get("statement_index"),
                    },
                }
            )
    ...
    seen = set()
    for x in sorted(
        out, key=lambda z: (z["interval_start"], z["interval_end"], z["source"], z["kind"])
    ):
...
def collect_h4_censors(p4: dict):
    ...
    for st in h4_all_statements(p4):
        ...
        fatal = any(
            x.get("matches_fatal_conflict_family") or FATAL_RE.search(x.get("english_label") or "")
            for x in causes
        )
    ...
    for st in h4_all_statements(p4):
        ...
        if s.get("usable") and not e.get("usable") and not (e.get("values") or []):
            out.append(
                {
                    "source_family": "H4",
                    "source": "H4_wikidata_current_open",
                    "direction": st.get("direction"),
                    "start": {
                        "precision": s["precision"],
                        "interval_start": s["interval_start"],
...
def strong_current_censor(h3markers: list[dict], h4markers: list[dict]):
    ...
    if len(signatures) < 2:
        return None
    return {
        "source": "strong_current_open_2026",
        "source_family": "H3_H4",
        "censor_type": "right_current",
        "precision": "day",
        "interval_start": SNAPSHOT_DATE,
        "interval_end": SNAPSHOT_DATE,
        "marker_signatures": [list(x) for x in sorted(signatures)],
        "markers": markers,
    }
...
def earliest_after_entry(items: list[dict], entry: dict):
    ...
    return (
        min(valid, key=lambda x: (x["interval_start"], x["interval_end"], x.get("source", "")))
        if valid
        else None
    )
...
def union_find_components(episodes: list[dict]):
    ...
    for ep in episodes:
        root = find(ep["person_a_id"])
        comps[root].append(ep)
    return comps
...
def main():
    ...
    _ = json.loads(FINAL.read_text(encoding="utf-8"))
    ub = {x["pair_key"]: x for x in u["pairs"]}
    b12 = {x["pair_key"]: x for x in h12["pairs"]}
    b3 = {x["pair_key"]: x for x in h3["pairs"]}
    b4 = {x["pair_key"]: x for x in h4["pairs"]}
    ...
    for pk in sorted(ub):
        ...
        if not model_ok:
            exclusions.append(
                {"pair_key": pk, "reason": "not_model_eligible_birth_swieph_duplicate_guard"}
            )
            counts["exclude_not_model_eligible"] += 1
            continue
        ...
        if not entry:
            exclusions.append(
                {
                    "pair_key": pk,
                    "reason": "no_usable_first_episode_entry",
                    "entry_candidates": entries,
                    "exits": exits,
                }
            )
            counts["exclude_no_entry"] += 1
            continue
        ...
        for r in p12.get("H1_relationship_ranges", []) or []:
            cand = {
                "source": "H1_finite_range_generic_end",
                "source_family": "H1",
                "censor_type": "generic_range_end",
                "precision": "year",
                "interval_start": r.get("interval_end_earliest") or r["interval_end"],
                "interval_end": r["interval_end"],
                "relationship_range": r,
            }
            if not any(overlap(cand, x) for x in exits):
                censors.append(cand)
        current = strong_current_censor(h3current.get(pk, []), collect_h4_current_markers(p4))
        if current:
            censors.append(current)
        censor = earliest_after_entry(censors, entry)

        outcome = None
        ambiguity = None
        if event and censor:
            if definitely_before(event, censor):
                outcome = "event"
            elif definitely_before(censor, event):
                outcome = "censor"
            else:
                ...
                if (
                    censor.get("censor_type") == "unknown_cause_H4"
                    and event.get("source") != "H4_wikidata_exact_pair_statement"
                ):
                    outcome = "event"
                else:
                    ambiguity = "event_censor_intervals_overlap"
        elif event:
            outcome = "event"
        elif censor:
            outcome = "censor"
            ...
            exclusions.append(
                {"pair_key": pk, "reason": "no_event_or_source_supported_censor", "entry": entry}
            )
            counts["exclude_no_outcome_or_censor"] += 1
            continue
        if ambiguity:
            exclusions.append(
                {
            ...
            counts["exclude_ambiguous_event_censor"] += 1
            continue

        final_end = event if outcome == "event" else censor
        entry_year = year_of(entry["interval_start"])
        end_year = year_of(final_end["interval_start"])
        if end_year < entry_year:
            exclusions.append(
                {
                    "pair_key": pk,
                    "reason": "end_year_before_entry_year",
                    "entry": entry,
                    "end": final_end,
                }
            )
            counts["exclude_negative_duration"] += 1
            continue
        if outcome == "censor" and end_year <= entry_year:
            exclusions.append(
                {
                    "pair_key": pk,
                    "reason": "censor_not_after_entry_year",
                    "entry": entry,
                    "censor": final_end,
                }
            )
            counts["exclude_zero_followup_censor"] += 1
            continue
        ...
        ep = {
            ...
            "outcome": outcome,
            "event": event if outcome == "event" else None,
            "censor": censor if outcome == "censor" else None,
            "end_year": end_year,
            "duration_years": end_year - entry_year,
            "relation_codes": relation_codes,
            "H3_current_markers": h3current.get(pk, []),
            "H4_current_markers": collect_h4_current_markers(p4),
        }
        episodes.append(ep)
        counts[f"accepted_{outcome}_episodes"] += 1
        for _year in range(entry_year, end_year + 1):
        ...
        pair_rows.append(
            {
                ...
                "all_clean_nonfatal_exits": exits,
                "candidate_censors": censors,
                "selected_outcome": outcome,
                "selected_event": ep["event"],
                "selected_censor": ep["censor"],
            }
        )
            ...
            pair_year_rows.append(
                {
                    ...
                    "person_a_id": ep["person_a_id"],
                    "person_b_id": ep["person_b_id"],
                    "calendar_year": y,
                    "duration_since_entry_year": y - ep["entry_year"],
                    "event": int(ep["outcome"] == "event" and y == ep["end_year"]),
                    "final_row": int(y == ep["end_year"]),
                    "final_row_type": ep["outcome"] if y == ep["end_year"] else None,
                    "relation_codes": ep["relation_codes"],
                    "entry_source": ep["entry"]["source"],
                    "entry_precision": ep["entry"]["precision"],
                }
            )
    ...
    for root, eps in sorted(comps.items(), key=lambda kv: str(kv[0])):
        ev = sum(e["outcome"] == "event" for e in eps)
        rows = sum(e["duration_years"] + 1 for e in eps)
        if ev:
            event_components += 1
        component_summary.append(
            {
                ...
                "events": ev,
                "pair_year_rows": rows,
                "pair_keys": sorted(e["pair_key"] for e in eps),
            }
        )

    event_pairs = sum(e["outcome"] == "event" for e in episodes)
    censored_pairs = sum(e["outcome"] == "censor" for e in episodes)
    nrows = len(pair_year_rows)
    gates = {
        "event_pairs_ge_50": event_pairs >= 50,
        "censored_non_event_pairs_ge_30": censored_pairs >= 30,
        "pair_year_rows_ge_200": nrows >= 200,
        "event_components_ge_5": event_components >= 5,
    }
    ready = all(gates.values())
    out = {
        "status": "development_dissolution_episode_readiness_source_only",
        "model_spec": str(SPEC.relative_to(REPO)),
        "model_spec_sha256": sha256(SPEC),
        "input_hashes": {
            "universe": sha256(UNIVERSE),
            "H1_H2": sha256(H12),
            "H3": sha256(H3),
            "H4": sha256(H4),
            "final_history_audit": sha256(FINAL),
        },
        "source_snapshot_date": SNAPSHOT_DATE,
        "H3_current_marker_fetch_failures": h3fail,
        "counts": dict(sorted(counts.items())),
        "readiness": {
            "accepted_model_eligible_episodes": len(episodes),
            "event_pairs": event_pairs,
            "censored_or_competing_non_event_pairs": censored_pairs,
            "pair_year_rows": nrows,
            "connected_components": len(comps),
            "event_containing_components": event_components,
            "gates": gates,
            "READY_FOR_FEATURE_REGISTRY_FREEZE": ready,
            "next_action": "freeze_machine_readable_feature_registry_and_engine_audit"
            if ready
            else "STOP_do_not_fit_and_do_not_lower_gate",
        },
        "component_summary": component_summary,
        "episodes": episodes,
        "pair_year_rows": pair_year_rows,
        "exclusions": exclusions,
        "pair_source_audit": pair_rows,
        "limitations": [
            "Source-only development audit; no astrology/HD features calculated.",
            (
                "Current-open censoring requires at least two frozen stru"
                "ctured markers; absence of an exit alone is never treate"
                "d as ongoing."
            ),
            (
                "Primary timing resolution is annual; source day/month pr"
                "ecision is retained only for ordering and provenance."
            ),
            "Unknown/fatal ends are censors, not nonfatal-exit events.",
        ],
    }
    OUT.write_text(
        ...
        json.dumps(
            {
                ...
                "counts": out["counts"],
                "H3_current_marker_fetch_failures_n": len(h3fail),
            },
            indent=2,
        ),
        flush=True,
