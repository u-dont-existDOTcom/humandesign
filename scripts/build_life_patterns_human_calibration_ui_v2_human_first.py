#!/usr/bin/env python3
"""Build the Life Patterns v2 auditor UI for human judgment rather than machine bookkeeping.

This presentation-only layer runs after the verified plain/provenance builder. It preserves the
embedded private handoff and response contract while removing machine-facing friction from the
independent human first pass.
"""
...
def _replace_function(text: str, name: str, replacement: str) -> str:
    ...
    if start < 0:
        raise ValueError(f"human-first UI patch could not find function {name}")
    candidates = [
        text.find("\nfunction ", start + 1),
...
def _embedded_fragment(html: str) -> str:
    ...
    if start < 0 or end < 0:
        raise ValueError("human-first UI patch cannot locate embedded handoff payload")
    return html[start:end]


def patch_html_for_human_first_calibration(html: str) -> str:
    """Remove nonessential human burden and make conditional judgments genuinely conditional."""
...
.quotesnippet{display:block;color:#46554f;margin-top:2px;line-height:1.35}
.relationchoice{display:grid;gap:8px;margin-top:9px}
.relationchoice label{display:flex;gap:8px;align-items:flex-start;padding:10px 11px;border:1px solid #d4ddd8;border-radius:9px;background:#fff}
.orderbox{margin-top:10px;padding:11px 12px;border:1px solid #d8e2dc;border-radius:9px;background:#f8fbf9}
.orderbox h4{margin:0 0 8px}.ordereditem{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:8px 0;border-top:1px solid #e5ece8}.ordereditem:first-of-type{border-top:0}
.ordereditem .orderlabel{font-weight:650}.ordereditem button{min-width:34px}
.humanerror{margin:7px 0}
    ...
    html = _replace_function(
        html,
        "readForm",
        r'''function readForm(u){const base=draftFromFormSeed(u);base.state=document.querySelector('input[name="state"]:checked')?.value||null;base.supporting_source_segment_ids=sourceSelections("support");base.counterevidence_source_segment_ids=[];base.context_qualifiers=[];base.missingness_flags=[];base.life_phase_qualifier=null;base.annotation_note=null;if(u.kind==="episode"){base.language=null;base.influence_relation="none_reported";base.influence_source_segment_ids=[]}if(base.state!=="observed")return base;let vals=selectedValues();let rel=vals.length===1?"single":document.querySelector('input[name="valueRelationHuman"]:checked')?.value||null;if(rel==="ordered_sequence"){const over=orderOverrides.get(u.key)||[];const set=new Set(vals);if(over.length===vals.length&&over.every(x=>set.has(x)))vals=[...over]}base.coded_values=vals;base.value_relation=rel;base.asserts_non_action=vals.some(v=>u.ext.non_action_values.includes(v));if(base.asserts_non_action){base.non_action_gate={};document.querySelectorAll('[data-gate]').forEach(x=>base.non_action_gate[x.dataset.gate]=x.value||null)}if(vals.includes(u.ext.other_specified_value))base.other_specified_description=$("osDescription")?.value.trim()||null;if(u.kind==="series"){base.reported_recurrence_strength=$("recurrenceStrength")?.value||null;base.exception_status=$("exceptionStatus")?.value||null;base.exception_frequency=base.exception_status==="exceptions_explicitly_denied"?"none_reported":base.exception_status==="exceptions_reported"?($("exceptionFrequency")?.value||"unknown"):null;base.frequency_evidence_basis=base.reported_recurrence_strength==="bounded_rate_or_count"?"bounded_rate_or_count_self_report":"generalized_self_report";base.minimum_reported_occurrences=null;base.bounded_rate_or_count_description=base.reported_recurrence_strength==="bounded_rate_or_count"?($("boundedDescription")?.value.trim()||null):null;base.recurrence_scope_description=null}return base}''',
    )

    html = _replace_function(
        html,
        "validate",
        r'''function validate(u,r){const e=[];const sourceIds=new Set(u.task.exact_source_segments.map(s=>s.segment_id));for(const arr of [r.supporting_source_segment_ids||[],r.counterevidence_source_segment_ids||[],r.influence_source_segment_ids||[]])for(const id of arr)if(!sourceIds.has(id))return["This saved response no longer matches this unit. Stop and report the technical problem."];if(!["observed","insufficient","not_applicable"].includes(r.state))e.push("Choose Yes, No, or Can't tell.");if(r.state==="observed"){if(!r.coded_values.length)e.push("Choose at least one behavior from the list.");if(r.coded_values.length===1&&r.value_relation!=="single")e.push("The page could not save the single behavior correctly. Reload this file and try again.");if(r.coded_values.length>1&&!['ordered_sequence','unordered_multiple'].includes(r.value_relation))e.push("You selected more than one behavior. Tell us whether the story gives a clear order.");if(r.value_relation==="ordered_sequence"&&r.coded_values.length<2)e.push("An ordered sequence needs at least two selected behaviors.");for(const v of r.coded_values)if(!u.obs.allowed_values.includes(v))return["The saved behavior selection is invalid for this question. Stop and report the technical problem."];if(!r.supporting_source_segment_ids.length)e.push(u.task.exact_source_segments.length>1?"Choose the quote(s) you relied on for this Yes answer.":"The page could not attach the exact quote to your Yes answer. Reload this file and try again.");const expectedNon=r.coded_values.some(v=>u.ext.non_action_values.includes(v));if(expectedNon&&(!r.non_action_gate||Object.values(r.non_action_gate).some(x=>x!=="established")))e.push("For the selected ‘did not act’ behavior, all four extra checks must be Yes. If any one is No or Can't tell, choose another behavior or Can't tell for the unit.");const os=r.coded_values.includes(u.ext.other_specified_value);if(os&&!r.other_specified_description)e.push("Describe the behavior that was not listed.");if(u.kind==="series"){if(!r.reported_recurrence_strength)e.push("Choose how often the narrator says this behavior recurs.");if(!r.exception_status)e.push("Choose whether the narrator reports exceptions or limits.");if(r.reported_recurrence_strength==="bounded_rate_or_count"&&!r.bounded_rate_or_count_description)e.push("Enter the count or rate that the narrator actually states.")}}return e}''',
    )
    ...
    if _embedded_fragment(html) != embedded_before:
        raise ValueError("human-first UI patch altered embedded private handoff bytes")
    forbidden = (
        "Optional uncertainty / context notes",
        "Narrator influence / precedence",
    ...
    if any(value in html for value in forbidden):
        # EP-002-SEG-01 may exist inside the embedded private handoff, so only reject it outside it.
        outside = html.replace(_embedded_fragment(html), "")
        for value in forbidden:
            if value != "EP-002-SEG-01" and value in outside:
                raise ValueError(f"human-first UI still exposes obsolete human-facing text: {value}")
    return html


def build_human_first_standalone_calibration_ui_v2(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="life-patterns-ui-v2-human-first-") as temporary:
        ...
        after = patch_html_for_human_first_calibration(before)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(after, encoding="utf-8", newline="\n")
    raw = output.read_bytes()
    receipt = dict(base_receipt)
    receipt.update(
        schema_version="life-patterns-human-calibration-ui-human-first-build-receipt-v2",
        output_html_sha256=hashlib.sha256(raw).hexdigest(),
        output_html_bytes=len(raw),
        optional_auxiliary_human_fields_shown=False,
...
def main() -> None:
    ...
    receipt = build_human_first_standalone_calibration_ui_v2(
        args.handoff_zip,
        args.output_html,
