...
def patch_html_for_final_auditor(html: str) -> str:
    ...
    html = html.replace(
        "</head>",
        """<style>
#advancedContext{display:none!important}.value code{display:none!important}.machineonly{display:none!important}
...
.ordereditem{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:8px 0;border-top:1px solid #e5ece8}.ordereditem:first-of-type{border-top:0}.ordereditem .orderlabel{font-weight:650}.ordereditem button{min-width:34px}.humanerror{margin:7px 0}
</style>
</head>""",
        1,
    )
...
let orderOverrides=new Map();function moveValue(v,dir){const u=units[current];const vals=[...(orderOverrides.get(u.key)||selectedValues())];const i=vals.indexOf(v),j=dir==="up"?i-1:i+1;if(i<0||j<0||j>=vals.length)return;[vals[i],vals[j]]=[vals[j],vals[i]];orderOverrides.set(u.key,vals);renderOrdered()}''',
    )
    html = _replace_function(
        html,
        "renderCommon",
