...
bad = (
    "def fetch_people_wikitext(people: dict[int, dict]) -> tu"
    "ple[dict[int, tuple[str, str]], list[dict]]]:"
)
good = (
    "def fetch_people_wikitext(people: dict[int, dict]) -> tu"
    "ple[dict[int, tuple[str, str]], list[dict]]:"
...
if bad not in source:
    raise SystemExit(
        "expected syntax-only annotation typo not found; audit runner before continuing"
    )
