...
def calc(x: float, body: int) -> tuple[float, float]:
    xx, ret = swe.calc_ut(x, body, FLAGS)
    used = ret & EPH_MASK
...
def design_jd(birth: float) -> float:
    lb, _ = calc(birth, swe.SUN)
    x = birth - 89.3
    for _ in range(10):
        ld, sp = calc(x, swe.SUN)
        f = ((lb - ld) % 360) - 88.0
        if abs(f) < 1e-10:
            return x
        x += f / max(abs(sp), 1e-9)
    ...
    def f(t):
        return ((lb - calc(t, swe.SUN)[0]) % 360) - 88.0

    flo = f(lo)
    for _ in range(80):
        mid = (lo + hi) / 2
        fm = f(mid)
        if abs(fm) < 1e-10 or (hi - lo) * 86400 < 0.1:
            return mid
        if flo * fm <= 0:
            hi = mid
        else:
...
def fingerprint(gates: set[int]) -> dict:
    ...
    label = {
        ...
        5: "5+4 Not a relationship anymore",
    }.get(n, f"{n}+{9 - n}")
    return {
...
def transit_gate_state(dt: datetime) -> tuple[dict[str, int], set[int]]:
    x = jd(dt)
    by = {name: gate(calc(x, b)[0]) for name, b in SLOW_TRANSITS.items()}
...
def exact_gate_events() -> list[float]:
    ...
    for name, b in SLOW_TRANSITS.items():
        ...
        while t < ej:
            ...
            g1 = gate(l1)
            if g1 != g0:
                # State bisection works even for retrograde crossings when one crossing occurs in
                # step.
                lo, hi = t, u
                old = g0
                for _ in range(50):
    ...
    ded = []
    for e in events:
        if not ded or abs(e - ded[-1]) * 86400 > 0.5:
            ded.append(e)
    return ded
...
def periods_for_pair(a_gates: set[int], b_gates: set[int], boundaries: list[float]) -> list[dict]:
    ...
    for a, b in zip(bounds[:-1], bounds[1:], strict=False):
        mid = dt_from_jd((a + b) / 2)
        by, tg = transit_gate_state(mid)
        fp = fingerprint(static | tg)
        row = {
        ...
        sig = (
            fp["defined_center_count"],
            tuple(fp["open_centers"]),
            fp["definition_components"],
            fp["surface_label"],
            tuple(fp["channels"]),
            tuple(sorted(by.items())),
        )
...
def main() -> None:
    ...
    for d in (
        ...
        for b in SLOW_TRANSITS.values():
            calc(jd(d), b)

    A_dt = datetime(1985, 1, 29, 10, 25, tzinfo=UTC)
    A = natal_gates(A_dt)
    douala = ZoneInfo("Africa/Douala")
    B_times = {
        "B_early": datetime(1989, 6, 19, 6, 0, tzinfo=douala).astimezone(UTC),
        "B_mid": datetime(1989, 6, 19, 13, 0, tzinfo=douala).astimezone(UTC),
        "B_late": datetime(1989, 6, 19, 18, 0, tzinfo=douala).astimezone(UTC),
    }
    ...
    for label, rec in bdata.items():
        print(
            label,
            "static",
            rec["static_connection"]["surface_label"],
            "components",
            rec["static_connection"]["definition_components"],
            "periods",
            len(rec["slow_transit_periods"]),
        )
