...
PORTABLE_SHA256_JS = (
    "function sha256Fallback(bytes){const K=new Uint32Array(["
    "1116352408,1899447441,3049323471,3921009573,961987163,15"
    "08970993,2453635748,2870763221,3624381080,310598401,6072"
    "25278,1426881987,1925078388,2162078206,2614888103,324822"
    "2580,3835390401,4022224774,264347078,604807628,770255983"
    ",1249150122,1555081692,1996064986,2554220882,2821834349,"
    "2952996808,3210313671,3336571891,3584528711,113926993,33"
    "8241895,666307205,773529912,1294757372,1396182291,169518"
    "3700,1986661051,2177026350,2456956037,2730485921,2820302"
    "411,3259730800,3345764771,3516065817,3600352804,40945719"
    "09,275423344,430227734,506948616,659060556,883997877,958"
    "139571,1322822218,1537002063,1747873779,1955562222,20241"
    "04815,2227730452,2361852424,2428436474,2756734187,320403"
    "1479,3329325298]);const H=new Uint32Array([1779033703,31"
    "44134277,1013904242,2773480762,1359893119,2600822924,528"
    "734635,1541459225]);const n=bytes.length,padded=((n+9+63"
    ")>>6)<<6,data=new Uint8Array(padded);data.set(bytes);dat"
    "a[n]=128;const dv=new DataView(data.buffer),bits=n*8;dv."
    "setUint32(padded-8,Math.floor(bits/4294967296),false);dv"
    ".setUint32(padded-4,bits>>>0,false);const w=new Uint32Ar"
    "ray(64),rotr=(x,n)=>(x>>>n)|(x<<(32-n));for(let off=0;of"
    "f<padded;off+=64){for(let i=0;i<16;i++)w[i]=dv.getUint32"
    "(off+i*4,false);for(let i=16;i<64;i++){const a=w[i-15],b"
    "=w[i-2],s0=rotr(a,7)^rotr(a,18)^(a>>>3),s1=rotr(b,17)^ro"
    "tr(b,19)^(b>>>10);w[i]=(w[i-16]+s0+w[i-7]+s1)>>>0}let[a,"
    "b,c,d,e,f,g,h]=H;for(let i=0;i<64;i++){const S1=rotr(e,6"
    ")^rotr(e,11)^rotr(e,25),ch=(e&f)^((~e)&g),t1=(h+S1+ch+K["
    "i]+w[i])>>>0,S0=rotr(a,2)^rotr(a,13)^rotr(a,22),maj=(a&b"
    ")^(a&c)^(b&c),t2=(S0+maj)>>>0;h=g;g=f;f=e;e=(d+t1)>>>0;d"
    "=c;c=b;b=a;a=(t1+t2)>>>0}H[0]=(H[0]+a)>>>0;H[1]=(H[1]+b)"
    ">>>0;H[2]=(H[2]+c)>>>0;H[3]=(H[3]+d)>>>0;H[4]=(H[4]+e)>>"
    ">0;H[5]=(H[5]+f)>>>0;H[6]=(H[6]+g)>>>0;H[7]=(H[7]+h)>>>0"
    '}return[...H].map(x=>x.toString(16).padStart(8,"0")).joi'
    'n("")}\nasync function sha256Hex(bytes){if(globalThis.cry'
    'pto&&crypto.subtle){const d=await crypto.subtle.digest("'
    'SHA-256",bytes);return[...new Uint8Array(d)].map(x=>x.to'
    'String(16).padStart(2,"0")).join("")}return sha256Fallba'
    "ck(bytes)}"
)
