...
def fetch_people_wikitext(people: dict[int, dict]) -> tuple[dict[int, tuple[str, str]], list[dict]]:
    ...
    for i, aid in enumerate(unresolved, 1):
        ...
        if found:
            ...
            failures.append(
                {"adb_id": aid, "name": p.get("name"), "public_title": p.get("public_title")}
            )
        if i % 25 == 0:
...
def merge_same_event(items: list[dict]) -> list[dict]:
    ...
    for ev in sorted(
        items,
        key=lambda x: (x["interval_start"], x["interval_end"], x["event_kind"], x["source_adb_id"]),
    ):
        match = None
        for g in groups:
...
def range_exit(
    ...
    if not (rule_a or rule_b):
        return "excluded", {
            ...
            "reason": "neither_H2_rule_A_nor_rule_B",
            "later_a": la,
            "later_b": lb,
        }
    item = {
        "transition": "nonfatal_exit",
        "source": "H2_adb_finite_range",
        "interval_start": lo,
        "interval_end": hi,
        "precision": "year",
...
def main():
    ...
    print(
        f"H1 exact ADB pages resolved {len(pages)}/{len(people)} failures={len(failures)}",
        flush=True,
    )
    ...
    h1_kind_counts = Counter()
    h1_precision = Counter()
    h1_transition = Counter()
    h2_rule_counts = Counter()
    pair_endpoint_count = 0
    reunion_pair_count = 0
    h2_accepted_count = 0
    h2_corroborating_count = 0
    h2_excluded_count = 0
    ...
    for idx, p in enumerate(pairs, 1):
        ...
        for src, other in ((a, b), (b, a)):
            ...
            other_title = pages.get(
                other, (other_meta.get("public_title") or other_meta.get("name") or "", "")
            )[0]
            evs = v1raw.extract_events(
                src, title, wt, other, other_title, other_meta.get("name") or other_title
            )
            rng = v1raw.extract_ranges(
                src, title, wt, other, other_title, other_meta.get("name") or other_title
            )
        ...
        for r in all_ranges:
            ...
            if status == "accepted":
                accepted_h2.append(item)
                h2_accepted_count += 1
                ra, rb = (
                    item["rule_A_explicit_nonfatal_wording"],
                    item["rule_B_both_demonstrably_alive_later"],
                )
                h2_rule_counts["A_and_B" if ra and rb else "A_only" if ra else "B_only"] += 1
            elif status == "corroborating":
                corroborating_h2.append(item)
                h2_corroborating_count += 1
            else:
                excluded_h2.append(item)
                h2_excluded_count += 1
        ...
        exits = [
            ...
            {
                "source": "H2_adb_finite_range",
                "event_kind": "range_nonfatal_exit",
                "interval_start": x["interval_start"],
                "interval_end": x["interval_end"],
                "precision": x["precision"],
            }
    ...
    out = {
        ...
        "sufficiency_preview_not_model_authorization": {
            ...
            "note": (
                "V4 requires completing frozen H3/H4 history augmentation"
                " and the final broad-universe audit before any separate "
                "model specification is written."
            ),
        },
        "pairs": pair_rows,
        "limitations": [
            ...
            (
                "H1 uses exact structured ADB relationship/event template"
                "s and strict partner attribution; H2 uses only the froze"
                "n conservative finite-range rule."
            ),
