...
def main() -> None:
    ...
    for p in h3["pairs"]:
        ...
        h1_formations = [
            x
            for x in h12_by[pk].get("H1_merged_transitions", [])
            if x.get("transition") == "formation"
        ]
        ...
        for row in exact_statements.get(pk, []):
            ...
            ordered_episode = bool(
                start.get("usable")
                and end.get("usable")
                and start["interval_end"] < end["interval_start"]
            )
            if ordered_episode:
    ...
    out = {
        ...
        "P1534_label_occurrences_on_exact_pair_statements": dict(
            sorted(cause_label_counts.items())
        ),
        ...
        "final_state_history_counts": {
            ...
            (
                "model_eligible_pairs_with_strict_exit_then_later_same_partner_formation"
            ): reunion_model,
            "all_pairs_with_reunion_supported_by_H4_repeated_start": repeated_start_reunion_all,
            "model_eligible_pairs_with_reunion_supported_by_H4_repeated_start": (
                repeated_start_reunion_model
            ),
        },
        "frozen_gate_result": {
        ...
        "limitations": [
            ...
            (
                "A new H4 nonfatal exit requires usable P582 plus a P1534"
                " English label in the frozen nonfatal lexical families."
            ),
            "End times without qualifying end cause do not create new endpoint-bearing pairs.",
            (
                "A Wikidata repeated-start reunion requires a separate ex"
                "act-pair statement with usable ordered P580 and P582; P5"
                "80-only statements are not enough."
            ),
