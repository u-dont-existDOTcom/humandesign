...
def _validate_finding(row: dict[str, Any], expected_index: int) -> None:
    ...
    _require(
        int(match.group(1)) == expected_index,
        f"finding IDs must be sequential; expected OA-{expected_index:03d}",
    )
    _require(row["observable_id"] in OBSERVABLE_IDS, f"{row['finding_id']}: invalid observable_id")
    _require(
        isinstance(row["subcode_ids"], list) and row["subcode_ids"],
        f"{row['finding_id']}: subcode_ids must be a nonempty list",
    )
    _require(
        all(isinstance(v, str) and v for v in row["subcode_ids"]),
        f"{row['finding_id']}: invalid subcode_ids",
    )
    ...
    _require(
        ...
        f"{row['finding_id']}: invalid classification for finding_type",
    )
    _require(
        row["recommended_disposition"] in DISPOSITIONS,
        f"{row['finding_id']}: invalid recommended_disposition",
    )
    ...
    for key in ("exact_issue", "human_coding_consequence"):
        _require(
            ...
            f"{row['finding_id']}: {key} must be nonempty",
        )
...
def _validate_summary(summary: dict[str, Any], findings: list[dict[str, Any]]) -> None:
    ...
    _require(
        summary["schema_version"] == "life-patterns-subcode-overlap-absence-audit-v1",
        "summary: wrong schema_version",
    )
    _require(summary["observables_reviewed"] == 22, "summary: observables_reviewed must be 22")
    _require(
        summary["material_findings"] == len(findings),
        "summary: material_findings does not equal JSONL row count",
    )
    _require(summary["r05_disposition"] in SUMMARY_DISPOSITIONS, "summary: invalid r05_disposition")
    _require(
        isinstance(summary["human_calibration_safe_to_start_without_revision"], bool),
        "summary: safety flag must be boolean",
    )
    _require(
        isinstance(summary["blocking_findings"], list), "summary: blocking_findings must be a list"
    )
    finding_ids = {row["finding_id"] for row in findings}
    _require(
        all(item in finding_ids for item in summary["blocking_findings"]),
        "summary: blocking_findings contains an unknown finding ID",
    )
    _require(
        isinstance(summary["notes"], str) and summary["notes"].strip(),
        "summary: notes must be nonempty",
    )
    ...
    expected = {
        ...
        "substantive_codebook_overlap_count": disposition_counts[
            "versioned_codebook_clarification"
        ],
    ...
    _require(
        sum(expected.values()) == len(findings),
        "summary: disposition counts do not cover every material finding",
    )

    reviewed = {row["observable_id"] for row in findings}
    _require(
        reviewed == OBSERVABLE_IDS,
        f"findings do not cover all 22 observables; missing={sorted(OBSERVABLE_IDS - reviewed)}",
    )
