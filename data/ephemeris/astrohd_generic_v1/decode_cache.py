...
def decode(path):
    with open(path, "rb") as stream:
        raw = lzma.decompress(stream.read())
    ...
    q[0] = q0
    q[1:] = q0 + np.cumsum(d)
    return ((q / 1e5) % 360).astype(np.float64), step


def interpolate(path, start_jd, target_jd):
    a, step = decode(path)
    x = (np.asarray(target_jd) - start_jd) * 24 / step
    i = np.floor(x).astype(int)
    f = x - i
    i = np.clip(i, 0, len(a) - 2)
    b = a[i]
    c = a[i + 1]
    delta = ((c - b + 180) % 360) - 180
    return (b + f * delta) % 360
