...
def note(status: str = "open", source: str = "U1", gap: str = "GAP-period") -> dict[str, Any]:
    return {
        "gap_id": gap,
        "issue": "Period of the reported routine is unclear.",
        "status": status,
...
def test_old_answer_retains_its_actual_preceding_question() -> None:
    turns = [
        turn("A1", "assistant", "Which period are you describing?"),
...
def test_consecutive_user_turns_share_preceding_not_invented_question() -> None:
    turns = [
        turn("A1", "assistant", "What happened next?"),
        turn("U1", "user", "I waited."),
        turn("U2", "user", "Then I left."),
    ]
...
def test_empty_malformed_and_other_roles_do_not_become_participant_statements() -> None:
    turns = [
        turn("S", "system", "not evidence"),
        turn("A", "assistant", "Question?"),
        turn("U0", "user", " "),
        turn("U1", "user", None),
        turn("U2", "user", "yes"),
    ]
    ...
    assert (
        context_coverage(turns, participant_statement_index(turns))["participant_turns_total"] == 1
    )


@pytest.mark.parametrize(
    "status", ["open", "answered", "unknown", "declined", "not_asked_burden", "not_material"]
)
def test_dispositions_are_explicit_process_metadata(status: str) -> None:
    turns = [
        turn("U1", "user", "Source statement."),
...
def test_unknown_assistant_and_empty_sources_are_rejected(source: str) -> None:
    ...
    with pytest.raises(ValueError):
        validate_clarification_sources(
            (ClarificationNote.model_validate(note(source=source)),), turns
        )
...
def test_future_turn_does_not_retroactively_validate_an_old_note() -> None:
    turns = [
        turn("A1", "assistant", "Reply", clarification_notes=[note(source="U-later")]),
...
def test_bounded_log_reports_omitted_items() -> None:
    turns = [
        turn("U1", "user", "Source"),
        turn(
            "A1", "assistant", "Reply", clarification_notes=[note(gap=f"GAP-{i}") for i in range(3)]
        ),
    ]
