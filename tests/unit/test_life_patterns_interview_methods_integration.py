...
class NotesInterviewer:
    ...
    def respond(
        self, *, turns: list[dict[str, Any]], **_: Any
    ) -> tuple[module.InterviewerResult, dict[str, str]]:
        source_id = "missing" if self.invalid_reference else str(turns[-1]["turn_id"])
        return module.InterviewerResult(
            reply="We can leave the date uncertain.",
            episode_ready=False,
            coverage_focus="none_material",
            clarification_notes=(
                ClarificationNote(
                    gap_id="GAP-date",
                    issue="The date is not remembered.",
                    status="unknown",
...
async def _asgi(
    app: FastAPI, method: str, url: str, body: dict[str, Any] | None
    ...
    raw = b"".join(
        cast(bytes, r.get("body", b"")) for r in sent if r["type"] == "http.response.body"
    )
    return int(status), cast(dict[str, Any], json.loads(raw or b"{}"))


def request(
    app: FastAPI, method: str, url: str, body: dict[str, Any] | None = None
) -> tuple[int, dict[str, Any]]:
    return asyncio.run(_asgi(app, method, url, body))


def setup(
    tmp_path: Path, *, invalid_reference: bool = False
) -> tuple[FastAPI, LifePatternsFileStore, str, str]:
    store = LifePatternsFileStore(tmp_path / "private")
    app = module.create_life_patterns_interview_app(
        store=store,
        interviewer=NotesInterviewer(invalid_reference=invalid_reference),
        mapper=object(),
...
def test_new_session_persists_exact_opener_not_just_ui_text(tmp_path: Path) -> None:
    ...
    status, public = request(
        app, "GET", f"/api/life-patterns/interview/sessions/{sid}?token={token}"
    )
    assert status == 200
...
def test_notes_persist_but_do_not_count_as_evidence(tmp_path: Path) -> None:
    ...
    status, response = request(
        app,
        "POST",
        f"/api/life-patterns/interview/sessions/{sid}/turns",
        {
            "token": token,
            "message": "I do not remember the date.",
        },
    )
    assert status == 200
...
def test_invalid_source_rejected_after_user_message_is_saved(tmp_path: Path) -> None:
    ...
    status, response = request(
        ...
        {
            "token": token,
            "message": "This message must survive invalid AI output.",
        },
    )
    assert status == 502
...
def test_legacy_record_not_backfilled_or_relabeled(tmp_path: Path) -> None:
    ...
    status, _ = request(
        ...
        {
            "token": token,
            "message": "I still cannot remember the date.",
        },
    )
    assert status == 200
    saved = store.read(sid, token)
    ...
    assert all(
        t.get("source_kind") != "fixed_interview_opening" for t in saved["conversation_turns"]
    )
...
def test_provider_receives_paired_sources_and_gap_log_without_external_call(
    ...
    result = module.InterviewerResult(
        reply="Understood.", episode_ready=False, coverage_focus="none_material"
    )
    ...
    assert (
        supplied["participant_statement_index"][0]["preceding_interviewer_turn"]["turn_id"] == "A0"
    )
    ...
    assert (
        receipt["interview_prompt_sha256"]
        == hashlib.sha256(body["instructions"].encode()).hexdigest()
    )
