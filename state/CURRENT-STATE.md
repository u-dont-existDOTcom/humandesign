...
GitHub Actions run `34991996940`: **SUCCESS**.
...
**The recoverability criterion is defined, implemented, live, and freezeable, but it has NOT yet passed.**

The next irreducible evidence is a genuinely fresh run generated by the new v2 interview:

1. owner refreshes/reopens the development interview and completes the target-blind measurement flow;
2. click **Freeze/export measurement** and keep the downloaded JSON unchanged;
3. only after that local freeze, apply the historical owner AstroHD crosswalk/recovery procedure;
4. evaluate the produced date/time signature with the executable regression gate;
5. if recovery is worse, treat the instrument as failed and revise the measurement design rather than weakening the historical benchmark.
